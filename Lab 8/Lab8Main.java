...
public class Lab8Main {
     ...
     */
    public static void visitPointCountsSorted(Consumer<MyMapEntry<Point, Integer>> consumer, Comparator<MyMapEntry<Point, Integer>> comparator) {
        for (List<MyMapEntry<Point, Integer>> bucket: pointCounts.getBuckets()){
            bucket.sort(comparator);
            for (MyMapEntry<Point, Integer> entry: bucket){
                consumer.accept(entry);
            }
        }
    }
